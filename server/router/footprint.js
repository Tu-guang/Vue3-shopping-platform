...
router.post('/footprint/add', async (req, res) => {
    ...
    var data = await mongodb.insertMany('footprint', x); // 在这里加上await关键字
    // console.log(data)
    res.send({
        "code": 200,
        "msg": "操作成功",
...
router.post('/footprint/list', async (req, res) => {
    console.log(req.body);
    var data = await mongodb.getData('footprint', req.body.user_id); // 在这里加上await关键字
    // console.log(data)
    res.send({
        "code": 200,
        "msg": "操作成功",
...
router.post('/footprint/del/:id', async (req, res) => {
    console.log(req.body);
    const id = req.params.id;
    var data = await mongodb.deleteById('footprint', parseInt(id)); // 在这里加上await关键字
    if (data === true) {
        console.log(data)
        res.send({
            "code": 200,
