...
import FavoritesItem from "@/views/Member/components/FavoritesItem.vue";
import {ref, onMounted} from 'vue';
import {favoritesListAPI} from '@/apis/favorites'

const likeList = ref([])
const getLikeList = async () => {
  const res = await favoritesListAPI()
  likeList.value = res.result
  // likeList.value = likeList.value = [
  //   {
  //     "id": "4023100",
  //     "name": "历史的棋局",
...
const handleReflush = (item) => {
  console.log(item)
  likeList.value = likeList.value.filter(likes => likes.id !== item.id)
}
</script>
  ...
  <div class="home-overview">
    <div class="like-container">
      <div class="home-panel">
        <div class="header">
          <h4 data-v-bcb266e0="">我的收藏</h4>
        </div>
        <div v-if="likeList.length === 0" style="width: 100%">
          <el-empty description="暂无数据"/>
        </div>
        <div v-else class="favorites-list">
          <FavoritesItem v-for="likes in likeList" :key="likes.id" :likes="likes" @reflush="handleReflush"/>
        </div>
      </div>
    </div>
  </div>
